...
__global__ void updateGrid(float4* newPos, int* gridCells, int* gridCounters) {
	...
	int i = atomicAdd(&gridCounters[gIndex], 1); //returns original contents before added
	i = min(i, sp.maxParticles - 1); //
	gridCells[gIndex * sp.maxParticles + i] = index; //Each cell contains 50 integers. This makes the first integer in each cell equals to corresponding gIndex.
}
...
__global__ void updateNeighbors(float4* newPos, int* phases, int* gridCells, int* gridCounters, int* neighbors, int* numNeighbors, int* contacts, int* numContacts) {
	...
	if (index >= sp.numParticles) return;
	/*
	__shared__ int shared_gridCounters[3 * 1024];

	int3 pos = getGridPos(newPos[index]);  //the x,y,z coordinate in the (32,32,32) grid;;;;make_int3(int(pos.x / sp.radius) % sp.gridWidth, int(pos.y / sp.radius) % sp.gridHeight, int(pos.z / sp.radius) % sp.gridDepth);

	int gridIndexPrev = -1;
	int gridIndexNext = -1;
	int gridIndex = getGridIndex(pos); //grid index at this level;;;;(pos.z * sp.gridHeight * sp.gridWidth) + (pos.y * sp.gridWidth) + pos.x;

	shared_gridCounters[gridIndex - pos.z * sp.gridHeight * sp.gridWidth + 1024] = gridCounters[gridIndex];

	if (gridIndex - 32 * 32 >= 0){
	gridIndexPrev = gridIndex - 32 * 32;
	shared_gridCounters[gridIndex - pos.z* sp.gridHeight * sp.gridWidth] = gridCounters[gridIndexPrev];
	}

	if (gridIndex + 32 * 32 < sp.gridDepth * sp.gridHeight * sp.gridWidth){
	gridIndexNext = gridIndex + 32 * 32;
	shared_gridCounters[gridIndex - pos.z * sp.gridHeight * sp.gridWidth + 1024 + 1024] = gridCounters[gridIndexNext];
	}
	__syncthreads();
	...
	for (int y = -1; y < 2; y++) {
	for (int x = -1; x < 2; x++) {
	int3 n = make_int3(pos.x + x, pos.y + y, pos.z + z);   //neightbor's coordinates: x+-1, y+-1, z+-1
	int3 shared_n = make_int3(pos.x + x, pos.y + y, 1 + z);
	if (n.x >= 0 && n.x < sp.gridWidth && n.y >= 0 && n.y < sp.gridHeight && n.z >= 0 && n.z < sp.gridDepth) {  //32x32x32
	int gIndex = getGridIndex(n);  //absolute linear index in the grid
	int gIndex_shared = getGridIndex(shared_n);
	int cellParticles = min(shared_gridCounters[gIndex_shared], sp.maxParticles - 1);  //each cell originally contains int 1.
	for (int i = 0; i < cellParticles; i++) {
	if (numNeighbors[index] >= sp.maxNeighbors) return;  //these reads from numNeighbors are coalesced

	pIndex = gridCells[gIndex * sp.maxParticles + i]; //reads a gIndex
	...
	numNeighbors[index]++;  //each thread updating its only numNeighbors counter
