...
public:
	...
	virtual void init(tempSolver* tp, solverParams* sp) {
		...
		sp->bounds = make_float3(dims) * radius; //(3.2, 3.2, 3.2)
		sp->gridWidth = int(sp->bounds.x / radius); //32
		sp->gridHeight = int(sp->bounds.y / radius); //32 
		sp->gridDepth = int(sp->bounds.z / radius); //32
		//std::cout << sp->gridWidth << " " << sp->gridHeight << " " << sp->gridDepth << std::endl;
